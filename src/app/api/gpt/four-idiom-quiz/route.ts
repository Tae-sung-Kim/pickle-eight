...
export async function POST(req: NextRequest) {
  ...
  const targetIdioms = await getFourIdiomsByDifficulty(difficulty);

  if (!targetIdioms) {
    return NextResponse.json(
      {
        error: `'${difficulty}' 난이도에 해당하는 사자성어를 찾을 수 없습니다.`,
      },
      { status: 404 }
    );
  }
...
3.  **정답(answer)**: 반드시 '${targetIdioms.answer}'를 그대로 반환해야 합니다.
4.  **해설(explanation)**: '${targetIdioms.answer}'라는 사자성어의 뜻과 유래를 간결하고 명확하게 설명해야 합니다. 다른 불필요한 내용은 절대 추가하지 마세요.

[출력 형식 (JSON)]
{
  "question": "(생성된 문제)",
  "answer": "${targetIdioms.answer}",
  "hint": "(생성된 힌트)",
  "explanation": "(생성된 해설)"
}
`;
