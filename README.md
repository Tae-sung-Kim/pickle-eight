...
- **운영**
  - 크레딧 시스템(자정 리셋, 상한 80, 5분마다 +1 자동 리필)
...
- 연령/동의: 로또 경로에 Age Gate(연령 확인), 전역 Cookie Consent(동의 전 분석 제한)를 적용합니다.
...
- **기본 지급/상한**: 매일 자정(KST) 기준으로 기본 80 크레딧 지급, 최대 보유/사용 상한 80 유지
- **자동 리필**: 5분마다 +1 자동 충전(상한 80까지), 페이지 진입/액션 시 멱등적으로 보정됨
- **소비 비용**: 분석/시뮬레이터/고급 생성/CSV 등 기능별 차등 비용 적용
